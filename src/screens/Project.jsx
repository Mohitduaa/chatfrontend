...
const Project = () => {
    ...
    const send = () => {
        if (message.trim()) {
            ...
            appendOutgoingMessage(message, replyTo);
            setMessage('');
            setReplyTo(null);
    ...
    function appendIncomingMessage(messageObject) {
        ...
        ${messageObject.replyTo ? `<div class='reply-context bg-gray-200 p-2 rounded-md'><small>Replying to: ${messageObject.replyTo.message}</small></div>` : ''}
    ...
    function appendOutgoingMessage(message, replyTo) {
        const messageBox = document.querySelector('.message-box');
        ...
        newMessage.innerHTML = `
            ...
            ${replyTo ? `<div class='reply-context bg-gray-200 p-2 rounded-md'><small>Replying to: ${replyTo.message}</small></div>` : ''}
            <div class='flex justify-between gap-2 items-center'>
    ...
    const handleReply = (messageObject) => {
                ...
                <div className='h-full' style={{ backgroundImage: "url('images/two.png')", backgroundSize: 'cover' }}>
                    <div className="conversation-area pt-12 pb-2 flex flex-col h-full fixed relative">
                        ...
                        <div className="inputfiled w-full flex fixed pb-3 py-3 bottom-0 left-0 bg-white" style={{ height: `calc(var(--vh, 1vh) * 10)` }}>
                            {replyTo && (
                                <div className="reply-context bg-gray-200 p-2 rounded-md flex items-center justify-between w-full mx-2 mb-2">
                                    <span>Replying to: {replyTo.message}</span>
                                    <button onClick={() => setReplyTo(null)} className="text-red-500 font-bold">X</button>
                                </div>
                            )}
                            <input
                                ref={inputRef}
